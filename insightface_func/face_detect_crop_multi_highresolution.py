...
class Face_detect_crop:
    ...
    def prepare(self, ctx_id, det_thresh=0.5, det_size=(640, 640), mode ='None', crop_size = 512, ratio = 0.7):
    ...
    def get(self, img, max_num=0, transformer_size = 40560):
        ...
        #     if max(width,height) > max_face:
        #         max_face = max(width,height)
        #         maxindex = i
        ...
        for i in range(bboxes.shape[0]):
            ...
            # torch_theta = cal_torch_theta(M, transformer_size, transformer_size, self.frame_h, self.frame_w).unsqueeze(0)
            # grid        = F.affine_grid(torch_theta, size=[1, 3, self.frame_h, self.frame_w])

            # ffhq_batch = F.grid_sample(ffhq_batch, 
            #             grid = mat_inv_list[i_batch:end,:,:,:].to(device),
            ...
            align_img   = Image.fromarray(align_img[:,:,[2,1,0]])
    ...
    def get_snap(self, img, scale_ratio=8, max_num=0):
        bboxes, kpss = self.det_model.detect(img,
                                             ...
                                             max_num=max_num,
                                             metric='default')
        crop_size = self.crop_size//scale_ratio
        ...
        for i in range(bboxes.shape[0]):
            ...
            width = bboxes[i][2] - bboxes[i][0]
            height = bboxes[i][3] - bboxes[i][1]
            if max(width,height) < self.min_size//scale_ratio:
                print("The detected face (%d,%d) is smaller than the minimum value %d"%(width,height,self.min_size))
                continue
            M, _        = face_align.estimate_norm(kps, crop_size, mode = self.mode) 
            align_img   = cv2.warpAffine(img, M, (crop_size, crop_size), flags=cv2.INTER_LINEAR, borderValue=0.0)
